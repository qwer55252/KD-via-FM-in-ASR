...
'''
# [<MUSIC>, <COMMA>, <NOISE>, <VOCALIZED_NOISE>, <LAUGHTER>, <SPOKEN_NOISE>, <PERIOD>, <COMMA>, <QUESTION_MARK>, <EXCLAMATION_MARK>, <SEMICOLON>, <COLON>, <DASH>, <ELLIPSIS>, <SIL>, <OTHER>]
def build_manifest_from_hf_gigaspeech(ds, manifest_path: str, cache_dir: str):
    ...
    with open(manifest_path, "w", encoding="utf-8") as fout:
        for sample in ds:
            ...
            if duration < min_sec:
                n_skipped += 1
                continue
            # text 필드 안전 처리 (Common Voice는 'sentence'가 일반적)
            raw_text = sample.get("sentence", None)
            if raw_text is None:
                raw_text = sample.get("text", "")
            text = normalize_text_cv(raw_text, keep_punct=False)
    ...
    print(f"[manifest] wrote {n_written} lines → {manifest_path}")
'''
